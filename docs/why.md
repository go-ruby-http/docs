...
`go-ruby-http/http` reimplements Ruby's `http` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-http/http` directly, with no Ruby runtime;
...
Because `http` is pure Go with cgo disabled, it also compiles to **WebAssembly**
